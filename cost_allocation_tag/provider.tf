...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      BusinessOwner = "DCS"
      Product       = "cost_allocation_tag"
      Repo          = "cost_allocation_tag"
      Tech          = "terraform"
      Environment   = "Prod"
    }
  }
...
terraform {
  ...
  backend "s3" {
    bucket = "cost_allocation_tag-tfgm"
    key    = "cost_allocation_tag/terraform.tfstate"
    region = "eu-west-1"
    profile = "ivan-arteaga-dev"
  }
}
